# Technischer Plan: API-Monitor & Mail-Notifier (plan_v1.1.md)
...
- **Struktur:** schlanke modulare Aufteilung mit `src/`-Layout
...
## 3. Projektstruktur (src-Layout)

Die Codebasis folgt einem klaren `src/`-Ansatz, damit Fachlogik sauber gekapselt und testbar bleibt.
...
Architekturregel:
- jede Domäne (`parser`, `matcher`, `storage`, `mailer`) hat einen eigenen Bereich direkt unter `src/`
- Orchestrierung liegt zentral in `src/loop/poll_loop.py`
- kein Mischcode zwischen Transport (`api`, `mailer`) und Fachlogik (`matcher`)

## 4. Python-Umgebung & Dependency-Strategie

Für diesen Plan ist die technische Grundlage fest definiert als:
- **Python `venv` + `requirements.txt`** (verbindlich für das Projektsetup)

Begründung:
- saubere, reproduzierbare Isolation der Abhängigkeiten pro Projekt
- standardnaher Python-Workflow ohne zusätzliche Toolchain
- ausreichend für dieses Projekt; Conda ist nicht vorgesehen

Umsetzungsprinzip:
- technische Basis bleibt möglichst nah an Built-ins (`sqlite3`, `smtplib`, `logging`)
- externe Pakete werden nur bei klarem Mehrwert in `requirements.txt` aufgenommen
...
## 6. Datenmodell / wichtige Entitäten

### 6.1 Persistenz (SQLite)
...
## 7. Schnittstellen und Contracts
...
## 8. Steuerung des Programmablaufs / Haupt-Workflow
...
## 9. Speicherung / Dateien / Datenbank
...
## 10. Fehlerbehandlung, Logging, Konfiguration
...
## 11. Technische Annahmen (Assumptions)
